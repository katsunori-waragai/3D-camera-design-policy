...
### python環境の標準化
- uvというツールがPython 環境構築の標準的なツールになってきている。
- requirements.txt やsetup.cfgを書く状況ではなくなってきている。
- pyproject.toml とか、uv で管理する状況になっている。
...
## scikit-image ベースの画像変換のGPU対応
- Pytorch, torchvision のライブラリには、scikit-imageとの対応がしやすい関数群が用意されています。
- すでにscikit-imageを利用した利用した関数がある場合には、簡単に移植が可能です。
- 元になる関数を示してChatGPUなどの生成AIにその範囲だけを書き換えさせるのです。
- これらの範囲では、既に多くの参照できる書き換え例があるために、生成AIに書き換えさせても問題なく動く場合が多い。
...
## 本番で使える画像認識にする心得
1. 学習環境とターゲット環境は異なることを自覚せよ。
2. １００回うまくいったからといって１０１回目がうまくいく保証はどこにもない。
3. 語られていない暗黙の想定が実運用にはある。
4. 失敗する可能性をあげつくしてみよう。
5. 