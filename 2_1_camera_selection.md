...
- MIPI(Camera Serial Interface)
  - カメラの画像信号処理基板からのインタフェースである。
- GMSL2
  - 自動車の自動運転用のカメラで使われていることが多い。
- GigEカメラ
  - 産業用カメラ分野
- その他多数
...
#### [ZED2,ZED-X カメラ ZED SDK(StereoLabs)](https://www.stereolabs.com/en-jp)
- [x] Windows 10/11
- [x] Ubuntu 20/22/24
いずれもCUDAデバイスがあること。
対象外
...
インテル RealSense は、RealSense, Inc に社名を変更しました
2025年7月11日、RealSense, Inc. は、インテルからのスピンアウトを完了しました。AI を活用したコンピューター・ビジョンのパイオニアである RealSense は、5,000 万ドルのシリーズ A 資金調達ラウンドを完了しました。この資金調達は、有名な半導体プライベート・エクイティ企業が主導し、インテル キャピタルや MediaTek Innovation Fund などの戦略的投資家が参加しました。プレスリリースを見る
...
### 成長できるコアがあるときに、成長のための痛みを回避してしまう。
- 技術には成長が起きるタイミングがある。
- そのタイミングに成長する際には、その分野を理解して、偏執的にものごとにとりくまなきゃならない。
- その分野の成長には、その分野の成長を成し遂げられる人を育てあげるか、招き入れるかだ。
- そのどちらもしなければ、その分野の変化から確実に取り残される。
#### 大多数の人は、楽な拡大しか知らない
- ガラケーが普及していったときの事業の拡大は、量的な拡大であって、質的なものではない。
- 携帯電話の製造が独自の事業部になっていったときは、量的な拡大である。
- 量的な拡大の範囲では、成長している企業の状況を経験して、「それが成長の秘訣」であると勘違いをしてしまう。
- 量的な拡大の範囲では、成長している結果を得られる場所に居合わせた人が、「優秀な人」として出世してしまう。
#### 質的な成長こそが大切
- その事業の中での質的な成長こそが大切
- その質的な成長を推進できる人に力を与えること
- 質的な成長こそが、優位性を創り出す。
#### カメラは世代交代し続ける定めにある。
- ある時点でベストな選択のカメラがあって、それに最適なカメラを選択しても、変わり続けなければならない定めにある。
- イメージセンサ・レンズ・カメラ信号処理の進展が、引き続き起こり続ける。
- カメラを利用するシステム設計者（ロボットの設計者も含む）はそのことを意識し続ける。
-                                                            

## 単眼カメラ
- ティアフォーカメラ
- [GMSL2インタフェース対応 HDRカメラ C1/C2](https://www.paltek.co.jp/solution/tier4/index.html)

## 経済圏・技術圏が別々になりかけているときに（バックアッププランの重要性）
- その部品は・ライブラリは、来年も輸入でいるものですか？
- もし、その部品やライブラリが入手不可能になったら、あなたのビジネスはどうなってしまいますか。
- 相互に依存性を高めてきた社会が、依存性を減らそうとする動きに転じている。
- 僕たちは、そのはざまのなかで大波にのまれないようにしていくこと。
- それぞれの勢力が自分の利害のために、ロビー活動をしている。
- 国際的な合意というのが常に妥当な内容になるわけじゃない。
- その部品・ライブラリは、いつまで入手可能ですか？
- 必要なバックアッププランをもっておく必要がある。
