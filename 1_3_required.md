...
## 起こりうる悪夢
- 補助金の範囲でだけ動くシステム
  - 「空飛ぶ自動車」って電動ヘリコプターの別名？
  - しかも、車検を通りそうもない。
  - 水素充填ステーション
  - 一部の都営バスとかしか、燃料電池車は走っていない。
  - 水素ガスの燃料電池の自動車が、ガソリンエンジン・ディーゼルエンジンを置き換える未来はやってこない。
- 補助金を喰い物にする一部の製品
  - 補助金によって、無理やりに市場を作り出している製品。
  - 投入した資本に対して、もとがとれるまでの期間が長すぎる。
  - 廃棄時のコストを計算に含めないことによる見かけの黒字
  - ライフサイクル評価（Life Cycle Assessment）を受け入れようとしないこと
  - 理念だけに基づく空論と間違えた政策
- 人を安い道具として使い潰してしまって、移行のための準備期間をなくしてしまうこと。
- 安い賃金であまんじてくれる「人不足」
  - 賃金などの労働条件を改善すれば、応募する人が増える。
  - その分野で、必要な資格を保持している人が多数いるのもかかわらず、慢性的な人不足の分野は、賃金などの労働条件の改善をすること。
- 開発成果・製造した製品・提供するサービスに対するしかるべき対価
  - 単に安さだけにしか価値をおかない製品開発は間違いである。
  - 安さ以上に重要なものに気づけない経営のもとで企業は劣化を進める。
- 安い労働力が確保できなくなった時点で、そのビジネスが終了してしまうこと。
- 安い労働力が確保できなくなったときには、ロボットで作業を置き換えるための投資をする体力も既になくなってしまっている。
- 安すぎる労働力を海外から取り入れようとする問題
  - エンジニアの価値を適切に評価できていない日本社会
  - 円安は、日本で稼ぐ魅力を損ねている。
  - 有能な人ほど英語圏でもっと稼げる場所に行ってしまう。
  - 奴隷的な労働
- 公費を投入して作り出した見かけ上の安い労働力
  - 他国から労働者を流入させるために、公費投入して作り出した見かけ上の安い労働力
  - 最低賃金未満の奴隷的労働条件
  - 多大な受け入れコスト
    - 言葉の習得問題
    - 自治体への負担の押し付け
    - 公立学校には、日本語を一切理解しない人を受け入れることはできない。
    - そのようなことを可能にする予算はない。
    - そのような対策で必要になるコストは、その人たち父母の労働によって利益を得る人たちが払う必要がある。
    - 
...
・一枚の画像から同時に複数の情報が得られるのが理想（しかも低遅延で）
　1枚の画像から、それぞれ別の画像認識を実施すると、それぞれの結果をつじつまのあう統合処理をしなくちゃならない。
　これは、単純ではない。
　ロボットのように応答性の高い動作を期待する場合には、本当の観測そのものの位置ではなく、
　観測後の経過時間を含めた予測位置に対して行動をおこさなくちゃならないからだ。
　ヒトを含む動物は、そのあたりの予測を込みで行動をしている。
　観測者である自身の位置や姿勢・顔向き・視線の向きを考慮する行動をとっている。
　ロボットが視覚に基づく行動をするためには、それらの情報を統合して、次の行動を生成しなくちゃならない。

・ヒューマノイドロボットの歩行動作では、視覚情報の寄与率は低い。
　画像認識から入ったエンジニアは、画像認識の重要度が高いと思いがちだが、
　最近のヒューマノイドロボットの歩行動作では、圧倒的に画像以外のセンサの情報を高頻度で制御をかけているはずだ。
　その中で動作生成モデルを訓練している。
　その部分が、シミュレーターで強化学習可能だから、現在の性能がでている。